...
Para la variable `salary`, que presentaba un porcentaje elevado de valores nulos y valores negativos que se transformaron a nulos para su posterior imputación, se decidió aplicar una imputación mediante `KNNImputer`. Esta imputación se realizó sobre la tabla `df_loyalty`, antes de unirla con la tabla de actividad, para evitar que la repetición mensual de clientes en la tabla conjunta distorsionara la estimación. De este modo, la imputación se llevó a cabo a nivel de cliente y no a nivel de registro mensual.
